...
app.post("/createPicture", async (req: Request, res: Response) => {
  ...
  try {
    ...
    span?.setAttributes({
      "app.phraseResponse": phraseText,
      "app.imageResponse": imageText,
    });
    const phraseResult = JSON.parse(phraseText);
    const imageResult = JSON.parse(imageText);
    createPictureSpan?.setAttributes({
      "app.phraseResult": phraseResult,
      "app.imageResult": imageResult,
    });
...
app.post("/createPictureWithSelfMeme", async (req: Request, res: Response) => {
  const span = trace.getActiveSpan();
  const createPictureSpan = tracer.startSpan("create picture");
  ...
  try {
    ...
    const phraseText = req.body.phrase;
